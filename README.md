...
| Chat and mobile UI | Intermediate reasoning and tool activity are grouped behind one `Show work` action. Mobile Enter inserts a newline, the send control is touch-friendly, active chats stay pinned to the latest messages, and background replies are prefetched. Composer drafts are persisted when the app is backgrounded. Working and delivery indicators recover from stale external turns. External HTTP(S) links have a Display setting for opening in a new tab; fork-specific file, session, and custom-protocol link protections remain in place. HTML artifacts sent through HAPI open in a new browser tab inside a sandboxed preview instead of requiring a download first. | The transcript stays compact and current, phone input behaves like a messaging app, drafts survive mobile app switching, and generated diagrams or reports are one tap away without receiving access to HAPI's authenticated origin. |
