...
| Codex defaults and lifecycle | New Codex sessions initially use YOLO mode; explicit later choices are remembered. Stop, reopen, restart, and Codex history sync preserve clearer lifecycle and activity timestamps. | Starting work is faster, and switching between HAPI and a local Codex session is less disruptive. **YOLO disables sandbox restrictions and ordinary approval prompts, but explicit project execpolicy `prompt` rules still reach HAPI.** |
...
- Treat YOLO mode as dangerous: it bypasses the agent's normal approval and sandbox protections except for explicit execpolicy `prompt` rules.
