...
describe('ApiMachineClient list-directory handler', () => {
    ...
    it('rejects browsing outside the home directory when no workspace roots are configured', async () => {
        const machine = {
            ...makeMachine('machine-ls-home-fallback'),
            metadata: {
                host: 'test',
                platform: 'linux',
                happyCliVersion: '0.0.0-test',
                homeDir: join(workspaceRoot, 'home'),
            },
        }
        const client = new ApiMachineClient('cli-token', machine)
        try {
