...
For Codex, `yolo` keeps the unrestricted `danger-full-access` sandbox and skips ordinary sandbox and MCP tool-call approval prompts, but still surfaces explicit project/user `execpolicy` rules whose decision is `prompt`. MCP forms and sign-in flows that require actual user input also remain interactive. This allows a repository to require HAPI approval for selected commands such as `git commit` and `git push` without prompting for normal work.
...
- **Antigravity** (`hapi agy`) — Google's Antigravity CLI (`agy`), driven headlessly via print mode: every turn spawns `agy -p <msg> --conversation <uuid> --output-format stream-json`, and NDJSON events (init / step_update / result) are streamed into the chat. There is no PTY/TUI wrapper and no hook-based permission bridge: permission handling uses agy's own `settings.json` allow/deny rules (`request-review`) or `--dangerously-skip-permissions` (`always-proceed`). Tool calls that lack an allow-rule are auto-denied by agy and surfaced as a chat hint. User MCP servers remain in `~/.gemini/config/mcp_config.json`; HAPI preserves them while adding one managed `hapi-session` entry whose target is selected from the current session environment. HAPI also adds a narrowly scoped allow-rule for `hapi-session/change_title`; other HAPI MCP tools retain Antigravity's normal permission behavior. This gives Antigravity the same session-scoped title and media tools without baking one session's bridge URL into global configuration. Remote-only — there is no local terminal input path. [Google Antigravity](https://antigravity.google)
