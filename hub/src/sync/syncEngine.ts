...
export class SyncEngine {
    ...
    async archiveSession(sessionId: string): Promise<void> {
        ...
        // #1911: machine-spawn stubs are exempt — any successful metadata write
        // releases the stub tag (updateSessionMetadata), which would make a
        // never-started stub unarchivable when StopSession can only answer
        // unknown. A stub has no live child, so there is no restart intent to
        // pin before stopping.
        const preArchiveRow = this.store.sessions.getSession(sessionId)
        if (!preArchiveRow || !isMachineSpawnPreallocatedStub(preArchiveRow)) {
